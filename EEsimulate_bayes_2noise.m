    ...
    for si = 1:data.nSubject
        ...
        for gi = 1:nT
            if nrans(si, horizon(gi)) > 0
                pdni = makedist('Logistic',0, sqrt(2)*nrans(si, horizon(gi)));
                dni = random(pdni);
            else
                dni = 0;
            end
            if isnan(n_det(repeatID(gi)))
                if ndets(si, horizon(gi)) > 0
                    pdne = makedist('Logistic',0, sqrt(2)*ndets(si, horizon(gi)));
        ...
        data.choice(si, :) = tool_extendnan(choice, size(data.choice, 2));
    end
    out = data;
end