function game = EEpreprocess_game_basic(game)
    disp('preprocessing');
    % basic info
    game = table_autofieldcombine(game);
    fn_game = fields(game);
    c = game.choice;
    r = game.reward;
    % get condition
    ...
    game.c_rp = [repmat(NaN,size(c_rp,1),1), c_rp];
    %%accuracy
    if any(strcmp(fn_game, 'trueMean'))
        game.side_correct = onetwoX(sign(game.trueMean(:,2) - game.trueMean(:,1)));
    else
        game.side_correct = NaN( size(game,1), 1);
    end
